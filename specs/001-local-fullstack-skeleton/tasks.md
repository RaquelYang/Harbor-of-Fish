...
- [ ] T001 建立 repository 根目錄 `.nvmrc`，內容為 `22`，選擇 Node 22 線；在 `frontend/package.json` 設定 `engines.node` 為 `>=22.12.0 <23`，明定最低 22.12.0 且限定 22.x，並使用 Angular 21、TypeScript 5.9.x、CLI 預設 Vitest/jsdom 相容版本。實作後於 repository 根目錄執行 `nvm use` 與 `node --version`，核對實際版本符合 `engines.node`；記錄實際輸出與比對結果，若選到較舊 22.x 須改用符合範圍的版本再檢查，命令未執行前標「待驗證」。
...
- [ ] T036 [US2] 核對 T013 已在 `backend/src/test/java/org/harboroffish/localtest/fixture/FlywayMigrationIT.java` 涵蓋 UUID/key 唯一、必要欄位、key/version pattern、非空文字、長度及 `test_only=true` 的 PostgreSQL 違規 INSERT；將這組既有 V1 限制案例列為 US2 回歸目標，不建立內容重複的 `LocalFixtureConstraintsIT.java` 或要求相同行為再次紅燈。若核對時發現具體未涵蓋的限制邊界，先記錄案例與現有案例差異，再補入既有測試。
- [ ] T037 [US2] 執行 `./mvnw -f backend/pom.xml -Dit.test=FlywayMigrationIT verify` 回歸 T036 確認的既有 V1 限制案例；Failsafe 報告須記錄實際執行名稱、數量與結果，命令尚未實測前維持待驗證。若發現 V1 限制缺口，不得修改已套用的 `V1__create_local_test_fixture.sql`，須新增有序 `V2__...sql`，並以 PostgreSQL 驗證：(1) 已套用 V1 的專用本機 DB 升級至 V2 並保留既有資料及 Flyway history；(2) 全新 DB 依序 V1→V2 後具備相同限制。不得清除或重建 DB 來掩蓋或修正升級問題。若沒有缺口，只記錄既有 V1 限制案例回歸通過，不新增空遷移或重複測試。
...
- [ ] T059 在根目錄 `README.md` 明確記錄本次完整驗收所選的單一目標 OS，並依該 OS 實際結果記錄 Node/Java/Docker/PostgreSQL 版本；其中 Node 須於根目錄執行 `nvm use`、`node --version`，核對實際版本符合 `frontend/package.json` 的 `engines.node`（`>=22.12.0 <23`），再記錄安裝與啟動順序、`infra/.env.local` 假值範例用法、DB 與 API 的實際 loopback 監聽位址及檢查命令；核對 `specs/001-local-fullstack-skeleton/quickstart.md` 的候選命令。逐條執行該目標 OS 適用的先決條件檢查、Maven Wrapper 首次下載/啟動、測試、載入、清除拒絕、冒煙及完整驗收命令，記錄實際命令和結果。執行允許清除或任何可能觸發資料刪除的命令前，須先列出具體命令與專用本機 DB 目標並取得使用者另行明確授權；未獲授權時不得執行，該命令及依賴它的驗收記為「待驗證」。macOS、Linux、Windows 中其他未實測 OS 須逐一記為「待驗證」，不得宣稱跨平台已驗收；只將實際成功者標已驗證，未實際執行的 quickstart 候選命令保持「待驗證」。
...
- [ ] T061 在根目錄 `README.md` 記錄繁中測試頁的瀏覽器驗收：窄螢幕、鍵盤、**重試控制的可見焦點**、載入／三筆成功／空集合／API 失敗／重試、應用資料請求只走 `/api/v1` 且不直接連資料庫；HTML、JavaScript、CSS 等靜態資源請求不計入此項資料請求檢查。空集合使用瀏覽器僅攔截 `GET /api/v1/local-test/fixtures` 的受控空 envelope，不清除既有 DB 資料；記錄攔截方式、回應內容、資料來源及解除攔截後的三筆結果，並與 T016 真實 API 空集合契約測試分開列示，不得將受控回應當成資料庫端到端證據。前端開發規範須記錄或直接連到此瀏覽器實測紀錄；無實際觀察的項目保留待驗證。
